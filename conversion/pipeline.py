...
def stage_translate(cfg, work):
    ...
    save(art(work, "2-english.json"), r)
...
def main() -> None:
    ...
    for stage in todo:
        ...
        if existing and existing.exists() and not a.force:
            print(f"[{stage}] artifact exists — skipping (use --force to redo)")
            continue
        print(f"[{stage}]")
